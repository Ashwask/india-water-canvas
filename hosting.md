...
## Path B · Custom domain `waterdashboard.com` (the requested path)

**URL you get:** `https://waterdashboard.com/Water/` or (with redirect) `https://waterdashboard.com/`

**Setup (30-60 minutes):**

### Step 1 · Register the domain
- Pick a registrar: Cloudflare Registrar (cheapest · ~$8-10/yr) · Namecheap (~$10-12/yr) · Google Domains
- Check availability: `waterdashboard.com` may or may not be free · if taken, alternates:
...
1. Visit `https://github.com/Ashwask/RFPartnerMap/settings/pages`
2. Under "Custom domain", enter: `waterdashboard.com`
...
If you want `https://waterdashboard.com/` to land directly on the water canvas (not the Partner Map dashboard at root):

**Option (a):** Add a small `index.html` at `/Water/index.html` so `https://waterdashboard.com/Water/` works → already done (see this commit · `Water/index.html` redirect to `dashboard.html`)
...
- `/CNAME` · placeholder for custom domain (replace `waterdashboard.com` with your actual domain when you set it up)
...
- **Apex domain (waterdashboard.com) vs www (www.waterdashboard.com):** both should work · GitHub auto-redirects between them when configured
